(function($){
    ...
    var browserCheck = function() {
        if(window.DataTransferItemList && window.File && window.FileReader && window.FileList && window.Blob && window.URL){
            return 2;//フォルダ対応
        }
        if (window.File) {
            return 1;//ファイル対応
    ...
    //読み込んだバイナリファイルの処理
    var bmsFileAnalyze = function(evt, file, dir) {
        var result = evt.target.result;
        fileNo = fileinfo[dir].length;
        fileinfo[dir].push(new Array());
        fileinfo[dir][fileNo]['filename']=file.name;
        fileinfo[dir][fileNo]['filesize']=file.size;

        //bmsonかどうかで読み込み関数を振り分け
        if (fileinfo[dir][fileNo]['filename'].slice(-5)==="bmson"){
            fileinfo[dir][fileNo]['bmson']=1;
            var ReadBMS=BMSONFileRead(DECODE_FILE(result),file.name);
        }else{
            fileinfo[dir][fileNo]['bmson']=0;
        ...
        for(i = 0; i < 16; i++) {
                ...
                fhash += "0123456789abcdef".charAt((c>>4) & 0xf);
                fhash += "0123456789abcdef".charAt(c & 0xf);
        }

        fileinfo[dir][fileNo]['title']=ReadBMS.title;
        fileinfo[dir][fileNo]['genre']=ReadBMS.genre;
        fileinfo[dir][fileNo]['playlevel']=ReadBMS.playlevel;
        fileinfo[dir][fileNo]['artist']=ReadBMS.artist;
        fileinfo[dir][fileNo]['player']=ReadBMS.player;
        fileinfo[dir][fileNo]['lanes']=ReadBMS.lanes;
        fileinfo[dir][fileNo]['maxbpm']=ReadBMS.maxbpm;
        fileinfo[dir][fileNo]['minbpm']=ReadBMS.minbpm;
        fileinfo[dir][fileNo]['difficulty']=ReadBMS.difficulty;
        fileinfo[dir][fileNo]['mode_hint']=ReadBMS.mode_hint;
        fileinfo[dir][fileNo]['chart_name']=ReadBMS.chart_name;
        ...
        if (doneQuery === requiredQuery && itemNumber === 0){
            var obj={
                files:fileinfo
            };
            
            readend = 1;
            settings["onRead"].call(parent, obj);
        }
    }


    //ファイルの読み込み
    var filesRead = function(e, _this) {
        _this.removeClass("bmsRead-dragenter");
        _this.addClass("bmsRead-reading");

        fileinfo['NODIR/'] = new Array();
        ...
        for (var i=0; i< files.length; i++) {
            ...
            reader.onerror = function (evt) {
                _this.append($("ファイル読み取りエラーが発生しました。"));
            };
            reader.onload = (function(file) {
                return function(evt) {
                    bmsFileAnalyze(evt, file, 'NODIR/');
                };
            })(f);

            var dot = f.name.slice(-3);
            var son = f.name.slice(-5);
            if (dot === "bme" || dot === "bms" || dot === "bml" || dot === "pms" || son === "bmson"){//BMSの形式だったら読み込み
    ...
    //フォルダの読み込み
    var foldersRead = function(e, _this) {
        _this.removeClass("bmsRead-dragenter");
        _this.addClass("bmsRead-reading");
        ...
        function ItemFileRead(item,path){
            item.file(function(file) {
                ...
                reader.onload = (function(file) {
                    return function(evt) {
                        itemNumber -= 1;
                        doneFile += 1;
                        settings["onProceed"].call(_this, totalFile, doneFile);
                        bmsFileAnalyze(evt, file, path);
                    };
                })(file);
                var dot = file.name.slice(-3);
                var son = file.name.slice(-5);
                if (dot == "bme" || dot == "bms" || dot == "bml" || dot == "pms" || son == "bmson"){
                    ...
                    if (itemNumber === 0 && readend === 0){                        
                        var obj={
                            files:fileinfo
                        };
                        readend = 1;
                        settings["onRead"].apply(obj);
                    }
                }
        ...
        var traverseFileTree = function self(item, path) {
            ...
            totalFile += 1;
            settings["onProceed"].call(_this, totalFile, doneFile);
            if (item.isFile) {//itemがファイルのとき
                if (path == ""){path="NODIR/"}
                if (!fileinfo[path]){
                    fileinfo[path] = new Array();
                };
                ItemFileRead(item,path);
            } else if (item.isDirectory) {//itemがディレクトリのとき
                ...
                var readEntries = function() {
                    dirReader.readEntries(function(results) {
                        if (!results.length) {
                            ...
                            entries = entries.concat(toArray(results));//直前まで読んでいたresultsに連結
                            readEntries();//Entryが帰ってこなくなるまで続ける
                        }
                    });
                };
                readEntries(); 
    ...
    var methods = {
        init : function( options ) {
            ...
            settings = $.extend(true, {
                'folderWait': 'Drag and Drop Files or Folders Here.',
                'fileWait':'Drag and Drop Files Here.',
                'noWait':'Your Browser doesn\'t support bmsRead.js',
                onDrop : function(){
                },
                onRead : function(fileinfo) {
                    this.children('.bmsRead_message').text("Finished");
                    console.log(fileinfo);
                },
                onProceed : function(totalFile, doneFile) {
                    this.children('.bmsRead_message').text("Reading Files: "+ doneFile +"/"+totalFile);
                }
            }, options);
            parent = this;
            this.addClass("bmsRead");
            this.append('<p class="bmsRead_message"></p>');
            //ブラウザのチェックとメッセージ表示
            switch (browserCheck()){
                case 1:
                    this.children('.bmsRead_message').text(settings.fileWait);
                    break;
                case 2:
                    this.children('.bmsRead_message').text(settings.folderWait);
                    break;
                default:
                    this.children('.bmsRead_message').text(settings.noWait);
                    return;
            }
            //ドロップイン時の挙動設定
            this.on("dragenter", function(e){
                $(this).addClass("bmsRead-dragenter");
                e.stopPropagation();
                e.preventDefault();
            });
            this.on("dragleave", function(e){
                $(this).removeClass("bmsRead-dragenter");
                e.stopPropagation();
                e.preventDefault();
            });
...
//BMSONデータ解読用関数
    function BMSONFileRead(result,filename){

        var bmshash=yjd_md5(result);

        var myData = JSON.parse(result);

        var title=myData.info.title;
        var genre=myData.info.genre;
        var playlevel=myData.info.level;
        var artist=myData.info.artist;
        var player=0;

        if ('mode_hint' in myData.info){
        var mode_hint=myData.info.mode_hint;
        }else{
        var mode_hint="";
        }


        var chart_name= ('chart_name' in myData.info) ? myData.info.chart_name : "";
        var difficulty=0;
        var lanes=0;


        var sound = ('soundChannel' in myData) ? myData.soundChannel : myData.sound_channels;
        var totalnotes=0;
        for (var i in sound){
            for (var j in sound[i].notes){
                totalnotes++;
            }
        }

        if ('init_bpm' in myData.info){
        var maxbpm=myData.info.init_bpm;
        }else{
        var maxbpm=myData.info.initBPM;
        }

        var minbpm=maxbpm;//

        if ('bpmNotes' in myData){
            var bpmevents=myData.bpmNotes;
        }else{
            var bpmevents=myData.bpm_events;
        }
        for (var i in bpmevents){
            if (bpmevents[i].bpm > maxbpm){
                maxbpm=bpmevents[i].bpm;
            }
            if (bpmevents[i].bpm < minbpm){
                minbpm=bpmevents[i].bpm;
            }
        }

        return {title:title,genre:genre,playlevel:playlevel,artist:artist,player:player,lanes:lanes,maxbpm:maxbpm,minbpm:minbpm,difficulty:difficulty,mode_hint:mode_hint,chart_name:chart_name,bmshash:bmshash,totalnotes:totalnotes};
    }


//BMSデータ解読用関数
    function BMSFileRead(result,filename){
        var bmshash=yjd_md5(result);

        var Tindex=result.indexOf('#TITLE');
        var Tend=Math.min(result.indexOf("\n",Tindex),result.indexOf("\r",Tindex));
        var title=result.slice(Tindex+7,Tend);//#TITLE属性を追加

        var Tindex=result.indexOf('#SUBTITLE');
        var Tend=Math.min(result.indexOf("\n",Tindex),result.indexOf("\r",Tindex));
        var subtitle=result.slice(Tindex+10,Tend);//#TITLE属性を追加

        var Tindex=result.indexOf('#GENRE');
        var Tend=Math.min(result.indexOf("\n",Tindex),result.indexOf("\r",Tindex));
        var genre=result.slice(Tindex+7,Tend);

        var Tindex=result.indexOf('#PLAYLEVEL');
        var Tend=Math.min(result.indexOf("\n",Tindex),result.indexOf("\r",Tindex));
        var playlevel=result.slice(Tindex+11,Tend);

        var Tindex=result.indexOf('#ARTIST');
        var Tend=Math.min(result.indexOf("\n",Tindex),result.indexOf("\r",Tindex));
        var artist=result.slice(Tindex+8,Tend);

        var Tindex=result.indexOf('#PLAYER');
        var Tend=Math.min(result.indexOf("\n",Tindex),result.indexOf("\r",Tindex));
        var player=result.slice(Tindex+8,Tend);
        if (player===""){player=1;}

        var Tindex=result.indexOf('#DIFFICULTY');
        var Tend=Math.min(result.indexOf("\n",Tindex),result.indexOf("\r",Tindex));
        var difficulty=result.slice(Tindex+12,Tend);
        if (difficulty===""){difficulty=0;}

        var lanes=0;
        if (/#\d\d\d18:/.test(result) || /#\d\d\d19:/.test(result)){lanes=1;}
        if (player!=1){lanes=2;}
        if (/#\d\d\d28:/.test(result) || /#\d\d\d29:/.test(result)){lanes=3;}
        if (filename.slice(-3)==="pms"){lanes=4;}

        var Tindex=result.indexOf('#BPM');
        var Tend=Math.min(result.indexOf("\n",Tindex),result.indexOf("\r",Tindex));
        var maxbpm=Number(result.slice(Tindex+5,Tend));//
        var minbpm=maxbpm;//
        ...
        var bpmnote=result.match(/#\d\d\d03:.+/g);
        var bpmnote2=result.match(/#BPM..\s.+/g);

        if (bpmnote){
        for (var i=0; i<bpmnote.length;i++){
        bpmnote[i]=bpmnote[i].replace(/#\d\d\d03:/,"");
        var two_chars = bpmnote[i].split(/(.{2})/).filter(Boolean);
        for (var j=0; j<two_chars.length;j++){
        if (two_chars[j]==="00"){continue;}
        maxbpm=Math.max(parseInt(two_chars[j],16),maxbpm);
        minbpm=Math.min(parseInt(two_chars[j],16),minbpm);
        }
        }}
        if (bpmnote2){
        for (var i=0; i<bpmnote2.length;i++){
        maxbpm=Math.max(bpmnote2[i]=bpmnote2[i].replace(/#BPM..\s/,""),maxbpm);
        minbpm=Math.min(bpmnote2[i]=bpmnote2[i].replace(/#BPM..\s/,""),minbpm);
        }}

        return {title:title,subtitle:subtitle,genre:genre,playlevel:playlevel,artist:artist,player:player,lanes:lanes,maxbpm:maxbpm,minbpm:minbpm,difficulty:difficulty,mode_hint:mode_hint,chart_name:chart_name,bmshash:bmshash,totalnotes:totalnotes};
    }

//その他の関数
    var DECODE_FILE = function(result){
        var filearray = new Uint8Array(result);
        switch (Encoding.detect(filearray)) {
            case 'UTF16':
                filearray = new Uint16Array(result);
                break;
            case 'UTF32':
                filearray = new Uint32Array(result);
                break;
        }
        var unicodeArray = Encoding.convert(filearray, 'UNICODE');
        var decodeResult = Encoding.codeToString(unicodeArray);
        return decodeResult;
    }

    $.fn.bmsRead = function( method ) {
        if ( methods[method] ) {
    ...
    function ArrayBufferToString(buffer) {
        var APSA=Array.prototype.slice.apply(new Uint8Array(buffer));
        var SSCT = "";
        for(var i=0,l=APSA.length; i<l; i++){
            SSCT += String.fromCharCode(APSA[i]);
        }
        return BinaryToString(SSCT);
    }
